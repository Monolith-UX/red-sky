 ...
 *
 * The development store, and the one the e2e suite reads directly. It holds
 * under `next dev` and a single `next start`, never on serverless hosting;
 * `store-supabase.ts` is the same set of functions over Postgres, and
 * `store.ts` chooses between them. Change both together.
 */
...
const a = globalThis as unknown as { __redSkyAttempts?: Map<string, number[]> };
const attempts = (a.__redSkyAttempts ??= new Map<string, number[]>());

/** Records an attempt at `key`, or refuses it once `limit` fall inside the window. In memory here. */
